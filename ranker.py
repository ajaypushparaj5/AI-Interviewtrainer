...
def rank_user_behavior(report, facial_expression_score, strictness=1, abnormal_thresholds=None):
    ...
    hands_clenched = norm(report.get("hands_clenched_count", 0))
    hands_behind_back = norm(report.get("hands_behind_back_count", 0))
    hands_in_pockets = norm(report.get("hands_in_pockets_count", 0))
    bpm = report.get("final_bpm", 0)
